...
void * printer_main(void * arg)
{
    ...
    while (!done) {
        // Lock the queue to safely check and fetch jobs
        ...
        // Unlock before printing so other threads can fetch while this one prints
        pthread_mutex_unlock(&jq->mutex);
...
int main(int argc, char *argv[])
{
    ...
    // Using dynamically allocated memory to avoid VLA warnings in stricter C standards, 
    // but a standard array is fine for this scope.
    ...
    /* prepare arguments to threads and create threads */
    for (i = 0; i < num_printers; i++) {
        printers[i].id = i;
        printers[i].jq = &job_queue;
        printers[i].njobs = 0;
        int rv = pthread_create(&printers[i].thread_id, NULL, printer_main, &printers[i]);
        check_pthread_return(rv, "pthread_create");
    }

    /* wait for other threads */
    for (i = 0; i < num_printers; i++) {
        int rv = pthread_join(printers[i].thread_id, NULL);
        check_pthread_return(rv, "pthread_join");
    }

    // Destroy the mutex after all threads are finished
    pthread_mutex_destroy(&job_queue.mutex);
